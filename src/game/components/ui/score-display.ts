...
// Constantes para cálculo de efectos (se mantienen)
...
export class ScoreDisplay extends LitElement {
  ...
  @state() private _scoreColor = 'var(--gq-scoredisp-text-color-base, #f3f4f6)'; // Usa variable con fallback
  ...
  static styles: CSSResultGroup = css`
    ...
    #score-text-internal {
      ...
      min-width: 1ch;
      user-select: none;
      /* font-weight y color se aplican dinámicamente a través de _scoreColor y _scoreWeight */
      /* text-shadow se aplica dinámicamente mediante variables CSS inyectadas */
    }

    @keyframes pulseFlare {
      0%, 100% { text-shadow: var(--final-flare-shadow); opacity: 1; } /* Usará la variable final */
      50% { text-shadow: var(--final-flare-shadow-pulse); opacity: 0.85; } /* Usará la variable final */
    }
  ...
  render() {
    // Construir las sombras del flare usando las variables CSS globales
