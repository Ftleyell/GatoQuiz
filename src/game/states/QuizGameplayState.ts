    ...
    // *** Añadir campo opcional 'explanation' a la interfaz Question ***
    interface Question {
      id: number | string;
      text: string;
      ...
      difficulty: string;
      explanation?: string; // Hacer la explicación opcional
    }
    ...
    export class QuizGameplayState implements IState {
      ...
      private isWaitingForExplanationConfirm: boolean = false; // *** Nuevo estado interno ***
      ...
      enter(params?: any): void {
        ...
        this.isWaitingForExplanationConfirm = false; // Resetear estado
      ...
      exit(): void {
        ...
        // Resetear visuales de combo y fondo
        ...
        this.isWaitingForExplanationConfirm = false; // Asegurar reset
      }
      ...
      private handleCorrectAnswer(difficulty: string): void {
        ...
        if (selectedTemplateId) {
            try { this.gameManager.getCatManager().addCat(selectedTemplateId); }
            catch (error) { console.error(`Error al llamar a catManager.addCat:`, error); }
        }

        // *** LLAMAR A proceedToNextStep en lugar de scheduleNextQuestion ***
      ...
      private handleIncorrectAnswer(): void {
        ...
        let shieldSaved = false; // Añadir flag

        if (playerData.hasShield) {
            shieldSaved = true; // Marcar que el escudo salvó
         ...
         if (hintConsumedThisTurn && !shieldSaved) { // Resetear pista solo si no salvó el escudo
      ...
      /**
       * Decide si mostrar explicación o pasar a la siguiente pregunta.
       */
      private proceedToNextStep(): void {
          ...
          if (explanation) {
              ...
              this.uiManager.showExplanation(explanation, () => {
                  // Este callback se ejecuta cuando el usuario confirma la explicación
                  this.isWaitingForExplanationConfirm = false;
                  this.scheduleNextQuestion(500); // Pequeño delay antes de la siguiente
              });
          } else {
              // Sin explicación, programar siguiente pregunta directamente
              this.scheduleNextQuestion(1500); // Delay normal
          }
      }
      ...
      /**
       * Callback que se ejecuta cuando se hace clic en un botón de opción.
       */
      public handleOptionClick(selectedKey: string): void {
        // *** Ignorar clics si estamos esperando confirmación de explicación ***
        ...
        // Consumir carga de pista si se usó en esta pregunta
        ...
        if (isCorrect === true) {
            ...
            // *** LLAMAR A proceedToNextStep también en caso de error ***
            this.proceedToNextStep();
        }
      }
